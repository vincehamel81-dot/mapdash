...
(actual OpenStreetMap-derived road data, not a grid or an abstraction) while dodging/finding/
racing other players in real time across devices. Originally named "MapRun", renamed to
...
~15m (real T-intersections the data doesn't explicitly connect), merges near-duplicate nodes, drops
degenerate near-zero-length edges, and dedupes consecutive-duplicate points in every polyline (all
four of these were real bugs found via live testing, not preemptive hardening — see Known Bugs).
